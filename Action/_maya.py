...
'''
This module contains common used functions of Maya.
'''
...
class Maya(_common.Common):
    ...
    def exportAbc(self, path, singleFrame=False, frameRange=None, objects=[],
                  options=['-ro', '-stripNamespaces', '-uvWrite', '-worldSpace',
                           '-dataFormat ogawa']):
        ...
        print "abc1111111111111111111111"
    ...
    def reference(self, path, groupOption=2, displayMode=None, removeNamespace=None):
        '''
    ...
    def assignMaterials(self, mapping,dire={}, geoNamespace='', matNamespace=''):
            dir = dire
            print dir,"%%%%%%%%%%%%%%%%%%%%%%%%"
            '''
                            ...
                            for i in dir:
                                for num,j in enumerate(dir[i]):
                                    if shape.lstrip('|').split("|")[0].split("_")[0] in j:
                                        kwargs['destination'] = shape.lstrip('|').split("|")[0]+str(num+1)+"|"+("|").join(dst.split("|")[1:])
                                        self._cmds.defaultNavigation(**kwargs)
                    except:
                        pass
                            ...
                            for shot_name in obj_list_name:
                                if "." in shot_name:
                                    if ":" in shot_name and "[" not in shot_name:
                                        ...
                                        obj_list_long_name.append(self.getLongName(name_shot, removeNamespace=removeNamespace))
                                    else:
                                        
                                        obj_list_long_name.append(self.getLongName(shot_name, removeNamespace=removeNamespace))
                                else:
                                    obj_list_long_name.append(self.getLongName(shot_name, removeNamespace=removeNamespace))
